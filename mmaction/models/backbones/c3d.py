...
class C3D(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        x = self.conv1a(x)# [10,3,32,224,224]->10, 64, 32, 224, 224
        x = self.pool1(x) #10, 64, 32, 112, 112

        x = self.conv2a(x) #10, 128, 32, 112, 112
        x = self.pool2(x) #10, 128, 16, 56, 56

        x = self.conv3a(x)
        x = self.conv3b(x) #10, 256, 16, 56, 56
        x = self.pool3(x) #10, 256, 8, 28, 28

        x = self.conv4a(x) #10, 512, 8, 28, 28
        x = self.conv4b(x) #10, 512, 8, 28, 28
        x = self.pool4(x) #10, 512, 4, 14, 14

        x = self.conv5a(x) #10, 512, 4, 14, 14
        x = self.conv5b(x)
        x = self.pool5(x) #10, 512, 2, 8, 8

        x = x.flatten(start_dim=1)#[10, 65536]
        
        x = self.relu(self.fc6(x))
        x = self.dropout(x)# 32 ,4096
