...
class ResNetforVit(nn.Module):
    ...
    def forward(self, x):
        ...
        Returns:
            ...
            by the backbone.
        """
        x = self.conv1(x) # [320, 64, 112, 112]
        x = self.maxpool(x) # 320, 64, 56, 56
        outs = []
    ...
    def guide_loss(self, x_fuse, num_segs, coarse_seg):
        # coarse_seg b, h, w 
        losses = dict()
        ...
        #     # loss_guide = getattr(self, 'loss_' + str(i+1))(x_mask, torch.ones_like(x_mask) * .5)
        #     x_mask = x[~mask.bool()]
        #     # print(x_mask.shape)
        #     loss_guide = getattr(self, 'loss_' + str(i+1))(x_mask, torch.zeros_like(x_mask) * .5) * (1/2**(3-i))
        #     losses.update({'loss_' + str(i+1) : loss_guide})
        x = x_fuse.reshape((-1, num_segs) + x_fuse.shape[1:]) # b, t, c, h, w
        b, t, c, h, w = x.shape
        mask = nn.functional.interpolate(coarse_seg.unsqueeze(1).to(torch.uint8), size=(h, w), mode='nearest').unsqueeze(1)
        mask = mask.expand_as(x)
        x_mask = x[~mask.bool()]
            # print(x_mask.shape)
        loss_guide = getattr(self, 'loss_' + str(2+1))(x_mask, torch.zeros_like(x_mask) * .5) * (1/2**(3-2))
        losses.update({'loss_' + str(2+1) : loss_guide})
        return losses
