...
@RECOGNIZERS.register_module()
class Recognizer2DForward(Recognizer2D):
    def forward(self, imgs):
        batches = imgs.shape[0]
        imgs = imgs.reshape((-1, ) + imgs.shape[2:]) # 16 32 3 224 224
        num_segs = imgs.shape[0] // batches

        losses = dict()

        x = self.extract_feat(imgs) # features [256,512,7,7]S
        if self.backbone_from in ['torchvision', 'timm']:
            if len(x.shape) == 4 and (x.shape[2] > 1 or x.shape[3] > 1):
                # apply adaptive avg pooling
                x = nn.AdaptiveAvgPool2d(1)(x)
    ...
    def forward_feat(self, imgs):
        ...
        x = self.extract_feat(imgs) #[32, 512, 7, 7])
        if self.backbone_from in ['torchvision', 'timm']:
            if len(x.shape) == 4 and (x.shape[2] > 1 or x.shape[3] > 1):
                # apply adaptive avg pooling
                x = nn.AdaptiveAvgPool2d(1)(x)
            x = x.reshape((x.shape[0], -1))
            x = x.reshape(x.shape + (1, 1))
        cls_score = self.cls_head.forward_feat(x, num_segs)
        return cls_score