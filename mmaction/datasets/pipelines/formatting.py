...
class FormatShape:
    ...
    def __init__(self, input_format, collapse=False):
        ...
        if self.input_format not in ['NCTHW', 'NCHW', 'NCHW_Flow', 'NPTCHW', 'custom','1CNHW', 'CNHW','NHWC']:
            raise ValueError(
    ...
    def __call__(self, results):
        ...
        if self.input_format == 'NCTHW':
            ...
            # M' x C x L x H x W # L理解为连续帧
            # M' = N_crops x N_clips
        elif self.input_format == 'NCHW':
            imgs = np.transpose(imgs, (0, 3, 1, 2))
            # M x C x H x W
        elif self.input_format == 'CNHW':
            ...
            clip_len = results['num_clips']
            num_clips = results['clip_len']
            imgs = imgs.reshape((-1, num_clips, clip_len) + imgs.shape[1:])
            # N_crops x N_clips x L x H x W x C
            imgs = np.transpose(imgs, (0, 1, 5, 2, 3, 4))
            # N_crops x N_clips x C x L x H x W
