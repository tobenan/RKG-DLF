...
def compute_bins(
    ...
    **Usage**
    ...
    if not isinstance(X, Tensor):
        raise ValueError(f'X must be a PyTorch tensor, however: type(X)={type(X)}')
    if X.ndim != 2:
        raise ValueError(f'X must have exactly two dimensions, however: X.ndim={X.ndim}')
    if X.shape[0] < 2:
        raise ValueError(f'X must have at least two rows, however: X.shape[0]={X.shape[0]}')
    if X.shape[1] < 1:
        raise ValueError(f'X must have at least one column, however: X.shape[1]={X.shape[1]}')
    ...
    if tree_kwargs is None:
        ...
        if y.ndim != 1:
            raise ValueError(f'y must have exactly one dimension, however: y.ndim={y.ndim}')
        if len(y) != len(X):
            raise ValueError(
                f'len(y) must be equal to len(X), however: len(y)={len(y)}, len(X)={len(X)}'
            )
        if y is None or regression is None:
            raise ValueError(
                'If tree_kwargs is not None, then y and regression must not be None'
...
class _PiecewiseLinearEncodingImpl(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        if x.ndim < 2:
            raise ValueError(
                f'The input must have at least two dimensions, however: x.ndim={x.ndim}'
            )
